...
import { WREN_POSES, WREN_STRIP, HOLLOW_STATES, LYING_STATES, STATE_SPEC, VARIANTS, poseWren, poseHollowRig } from './rig-poses.mjs';
...
// ------------------------------------------------------------ posing (shared with the checks: rig-poses.mjs)
...
window.hollowSheet = async () => {
  ...
  VARIANTS.forEach(([v, name], r) => {
    ...
    HOLLOW_STATES.forEach((st, i) => {
      ...
      const lying = st === 'dormant' || LYING_STATES.includes(st);
